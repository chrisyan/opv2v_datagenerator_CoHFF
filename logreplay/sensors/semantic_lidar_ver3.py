...
class SemanticLidar(BaseSensor):
    ...
    def spawn_point_estimation(relative_position, global_position):
        ...
        if relative_position == 'front':
            ...
            carla_location = carla.Location(x=carla_location.x + 11.0, #8.5
                                            y=carla_location.y,
                                            z=carla_location.z + 1.0)
            yaw = 180

        elif relative_position == 'rightver3':
            carla_location = carla.Location(x=carla_location.x + 0.0,
                                            y=carla_location.y + 6.3,
                                            z=carla_location.z + 1.8)
            yaw = -90

        elif relative_position == 'leftver3':
            ...
            carla_location = carla.Location(x=carla_location.x - 8.0,
                                            y=carla_location.y,
    ...
    def data_dump_frontver3(self, output_root, cur_timestamp, sensor_name):
    ...
    # 将点云数据保存为pcd文件
            with open(save_path, 'w') as file:
                ...
                for point, tag in zip(self.points, self.obj_tag):
                    x, y, z = point
                    # 将点坐标和tag写入文件
                    file.write("{} {} {} {}\n".format(-x+11.5, -y, z-0.89, tag)) # -x+9.0

            print(f'Saved Semantic Lidar data to {save_path}')

    def data_dump_backver3(self, output_root, cur_timestamp, sensor_name):
        '''save semantic lidar as .pcd'''
        if True:  # self.points is not None:
            # 创建保存数据的目录（如果不存在）
            os.makedirs(output_root, exist_ok=True)
            ...
            with open(save_path, 'w') as file:
                ...
                for point, tag in zip(self.points, self.obj_tag):
                    x, y, z = point
                    # 将点坐标和tag写入文件
                    file.write("{} {} {} {}\n".format(-x-1.5, -y, z-0.5, tag))
            print(f'Saved Semantic Lidar data to {save_path}')

    def data_dump_rightver3(self, output_root, cur_timestamp, sensor_name):
        '''save semantic lidar as .pcd'''
        if True:  # self.points is not None:
            # 创建保存数据的目录（如果不存在）
            os.makedirs(output_root, exist_ok=True)
    ...
    def data_dump_leftver3(self, output_root, cur_timestamp, sensor_name):
        '''save semantic lidar as .pcd'''
        if True:  # self.points is not None:
            # 创建保存数据的目录（如果不存在）
            os.makedirs(output_root, exist_ok=True)
