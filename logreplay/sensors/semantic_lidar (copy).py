...
class SemanticLidar(BaseSensor):
    ...
    def spawn_point_estimation(relative_position, global_position):
        ...
        if relative_position == 'front':
            carla_location = carla.Location(x=carla_location.x +6, #2.5
                                            y=carla_location.y,
            ...
            carla_location = carla.Location(x=carla_location.x + 0.0,
                                            y=carla_location.y + 0.3-6.3,
                                            z=carla_location.z + 1.8)
            yaw = -100

        elif relative_position == 'left':
            carla_location = carla.Location(x=carla_location.x + 0.0,
                                            y=carla_location.y -0.3 +6.3, # -0.3
                                            z=carla_location.z + 1.8)
            yaw = 100 #-100
    ...
    # 将点云数据保存为pcd文件
            with open(save_path, 'w') as file:
                ...
                for point in self.points:
                    rotated_point = np.dot(rotation_matrix, point.T)
                    # x
                    # y
                    # z
                    rotated_point_cloud.append(rotated_point)
                   # print("rotated_point_cloud: ",rotated_point_cloud)

                print("rotated!!!!!!!!!!!!!!!!!!!!!!!!!")
                #rotated_point_cloud = np.dot(rotation_matrix, self.points.T)
        ...
        if True:  # self.points is not None:
            ...
            with open(save_path, 'w') as file:
                ...
                for point, tag in zip(rotated_point_cloud, self.obj_tag):
                    x, y, z = point
                    # 将点坐标和tag写入文件
                    file.write("{} {} {} {}\n".format(x + 0.49, y+0.315 -6.3, z - 0.099, tag))
            ...
            with open(save_path, 'w') as file:
                ...
                if True:
                    ...
                    for point, tag in zip(rotated_point_cloud, self.obj_tag):
                        x, y, z = point
                        # 将点坐标和tag写入文件
                        file.write("{} {} {} {}\n".format(x + 0.51, y - 0.2845 +6.3, z - 0.099, tag))
